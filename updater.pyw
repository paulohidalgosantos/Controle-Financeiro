...
class UpdaterGUI:
    ...
    def escrever_log(self, mensagem: str):
        """Escreve log em tela e em arquivo"""
    ...
    def atualizar_app(self):
        try:
            ...
            self.escrever_log(f"[ERRO] Falha na atualização: {e}")
...
def main():
    try:
        ...
        try:
            with open("update_log.txt", "a", encoding="utf-8") as f:
                f.write(f"[FATAL] Erro no updater: {e}\n")
        except:
            pass
        messagebox.showerror("Erro fatal", f"Erro inesperado no updater: {e}")
        sys.exit(1)
